...
export function createHomeProjectsController(home: HomeController) {
  ...
  async function disposeHomeProjectInstances(conn: ServerConnection.Any, project: LocalProject) {
    ...
    const dirs = projectWorkspaceDirectories(project)
    for (const directory of dirs) {
      ctx.sync.disposeDirectory(directory, { force: true })
    }
    await Promise.all(
      dirs.map((directory) => ctx.sdk.client.instance.dispose({ directory }).catch(() => undefined)),
    )
  }
  ...
  function performCloseHomeProject(conn: ServerConnection.Any, directory: string) {
    ...
    const next = closeHomeProject(home.selection.value(), ServerConnection.key(conn), ctx.projects, directory)
