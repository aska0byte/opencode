...
const createFailureEnv = AppNodeBuilder.build(SessionCompaction.node, [
  [SessionNs.node, failingPartSession],
  [Provider.node, defaultProvider.layer],
  [SessionProcessorModule.SessionProcessor.node, processorLayer("continue")],
  [RuntimeFlags.node, RuntimeFlags.layer({ experimentalEventSystem: true })],
])
  ...
  itLocal.live(
    ...
    Effect.gen(function* () {
      ...
      const effect = Effect.gen(function* () {
        ...
        Effect.provide(
          AppNodeBuilder.build(SessionCompaction.node, [
            [SessionNs.node, gatedSession],
            [Provider.node, defaultProvider.layer],
            [SessionProcessorModule.SessionProcessor.node, processorLayer("continue")],
            [RuntimeFlags.node, RuntimeFlags.layer({ experimentalEventSystem: true })],
          ]),
...
describe("util.token.estimate", () => {
  test("estimates tokens from ascii text with letter weight", () => {
    const text = "x".repeat(4000)
    expect(Token.estimate(text)).toBe(1080)
  })

  test("estimates tokens from larger text", () => {
    const text = "y".repeat(20_000)
    expect(Token.estimate(text)).toBe(5400)
  })
