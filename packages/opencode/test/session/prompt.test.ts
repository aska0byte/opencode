...
const compactingProcessorNode = LayerNode.make({
  service: SessionProcessor.Service,
  layer: compactingProcessor,
  deps: [Session.node],
})
...
function makePrompt(input?: { mcpInstructions?: MCP.ServerInstructions[]; processor?: "blocking" | "compact" }) {
  const replacements = [
  ...
  if (input?.processor === "compact") {
    return LayerNode.compile(promptRoot, [...replacements, [SessionProcessor.node, compactingProcessorNode]])
  }
  return LayerNode.compile(promptRoot, replacements)
}

function makeHttp(input?: { mcpInstructions?: MCP.ServerInstructions[]; processor?: "blocking" | "compact" }) {
  ...
  if (input?.processor === "compact") {
    return LayerNode.compile(root, [...replacements, [SessionProcessor.node, compactingProcessorNode]])
  }
  return LayerNode.compile(root, replacements)
}

function makeHttpNoLLMServer(input?: { mcpInstructions?: MCP.ServerInstructions[]; processor?: "blocking" | "compact" }) {
